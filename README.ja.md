...
最終レポートは人が読み通せる判断メモとして書きます。本文では結論、読み方、調査方法とフレームワーク、フレームワーク次元別分析、主要な物語、判断ロジック、最終推奨、結論を変える条件、次の行動、範囲を先に示します。証拠レジスター、情報源品質、レッドチームメモ、シナリオ、品質スコア、情報源一覧は付録に置きます。方法部分で次元を列挙した場合、その次元は本文の Markdown 小見出しとして現れる必要があり、方法メモや付録だけに置いてはいけません。本文が箇条書きや監査表に支配されている場合は最終ゲートを通過しません。
