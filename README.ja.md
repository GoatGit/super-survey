...
エボルバーはレポート品質スコアの前に実行します。これはラウンド単位のゲートで、最新の統合結論とレッドチーム批判を `維持 / 絞り込み / ピボット / 中止` と、より鋭い次回ラウンドの焦点に変換します。品質スコアはレポート単位のゲートで、更新後の `report.md` に対してのみ付けます。調査を停止できるのは両方のゲートを通過した場合だけです。つまり、レポートが選択モードのしきい値を満たし、エボルバーが明確な kill reason で仮説を中止するか、desk research で減らせる次の証拠が残っていない必要があります。エボルバーが `維持`、`絞り込み`、または `ピボット` を示し、desk research 可能な証拠を挙げている場合は、`report.md` がその証拠は外部検証のみ、または意思決定を変えないと明示しない限り、次のラウンドを続けます。
