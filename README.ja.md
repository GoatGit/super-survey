...
CLI はラウンドの足場ファイルをまとめて作成できますが、内容には厳密な依存順序があります。先に `NN-research.md` を書き、次に `NN-brainstorm.md`、`NN-redteam.md`、`NN-synthesis.md`、最後に `NN-evolver.md` を書きます。下流の成果物は、最終結論を予測するのではなく、すでに書かれた上流の発見を参照します。
...
```mermaid
flowchart TD
    A[ユーザーの調査問い] --> B[00-brief.md<br/>判断、最適化契約、調査レンズ、調査フレームワーク、証拠基準]
    ...
    F --> G[統合結論<br/>信頼度、判断根拠、感度変数、反事実]
    G --> H[エボルバー<br/>維持 / 絞り込み / ピボット / 中止 / 最終化<br/>Kill 範囲と元の問いの状態]
    H --> Q{エボルバー判断}
