...
`check` は、必要ファイルの欠落、見出しの欠落、必須セクションの空欄、空テンプレートのままの成果物、または v2 レポートに解析可能な品質スコアがない場合に失敗します。ラウンド番号は正の整数である必要があります。古い 6 セクションのレポートは warning 付きで互換扱いになります。`upgrade-report` を実行すると完全な report schema が追加されるため、新しいセクションを埋めてください。
...
- 完全な最終レポートとして独立した `report.md`。範囲、方法/情報源品質、証拠、分析、レッドチーム批判、選択肢/シナリオ、推奨事項、行動計画、未解決の問い、レポート品質スコア、制約、情報源メモを含める
