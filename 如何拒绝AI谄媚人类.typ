...
$lambda$ 和 $gamma$
不应被理解为可以精确估计的物理常数，而是把用户约束写入决策问题的规范化偏好系数。为了让不同报告可比较，实践中应先把
$U$、$R$、$I$ 归一到同一尺度，例如 0-3 的离散等级或 0-1
的粗粒度评分，再设置权重。一个可执行的默认规则是：

- 若任务低风险、可逆、试错成本低，可取 $lambda approx 1, gamma approx 1$，让收益、风险和信息不足保持相近权重；
- 若行动存在大回撤、不可逆损失、合规责任或高机会成本，应提高 $lambda$，使风险残差优先进入判断；
- 若关键证据缺失足以改变行动建议，应提高 $gamma$，使“继续调研”能战胜过早结论；
...
=== 3.1.1 与现有去偏方法的区别
<与现有去偏方法的区别>
现有去偏工作通常从模型训练或评测层面切入。指令微调、RLHF/RLAIF 和 Constitutional AI
试图让模型整体上更有帮助、更诚实、更无害；合成反谄媚数据试图降低模型迎合用户观点的概率；红队测试和对抗探测则通过攻击性输入发现模型的脆弱行为。这些方法很重要，但它们主要回答“如何让模型一般性地少犯错”。本文关注的是另一个层面：当模型已经进入一个具体的开放式调研任务时，如何防止它把用户的原始问题误当成目标函数。
...
#figure(
  align(center)[#table(
    ...
    table.header([方法], [主要作用层面], [主要优点], [对开放式调研的剩余缺口],),
    table.hline(),
...
前面的讨论已经足够说明，开放式调研不是在一个可微损失函数上求最小值，而是在一个由问题、约束、证据、假设和行动构成的离散状态空间里，持续压缩决策残差。本文中，“残差”指当前报告中尚未被压缩的决策缺口，例如目标未澄清、约束缺失、证据不足、反方过弱、敏感变量不清或行动边界不可执行；“残差度量”指对这些缺口进行离散评分和加权聚合的方法。本节只给出形式化语言：状态、残差、算子、下降方向和停止条件；具体如何执行一轮迭代，留到第五章展开。若要把这种迭代写成更统一的理论，可以把第 $t$ 轮调研状态记为：
...
#block[#text(weight: "semibold")[十、贝叶斯更新]]
...
#block[#text(weight: "semibold")[十一、综合判断与决策树]]
...
#block[#text(weight: "semibold")[十二、残差度量]]
...
#block[#text(weight: "semibold")[十三、下一轮方向或停止理由]]
