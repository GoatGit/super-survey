...
本文提出一个通用方法：把开放式调研从"回答生成"重构为"带约束的决策优化"。该方法要求
agent
在收敛结论之前，先重建目标函数、约束条件、事实与假设边界、隐含预期、反事实情景、反方论证和更新规则。英伟达案例只是
running
example；同一框架也适用于产品机会、市场进入、技术选型、开源项目比较和尽职调查。

本文的贡献有三点。第一，给出"AI
谄媚"在开放式调研中的操作性定义：接受用户原始问题作为目标函数，并将回答优化为用户更容易接受的局部最优。第二，用约束优化、鲁棒优化、敏感性分析和贝叶斯更新解释如何抵抗这种局部最优。第三，给出可复用的调研流程、质量评估方法和局限性说明，使该方法能够落地到多轮调研工作流中。

关键词：AI
sycophancy；开放式调研；约束优化；鲁棒决策；贝叶斯更新；反事实分析；决策树
...
+ 什么样的流程能让 agent 在多轮调研中持续更新，而不是先下结论再找论据？
+ 如何评估一份调研报告是否真正服务于人类决策者，而不是只留下任务审计痕迹？

本文的基本立场是：世界是随机、嘈杂、不可可靠预测的。调研的目标不是制造确定性，而是在不确定性下改善判断质量。一个好的
agent
不应替用户下注，而应帮助用户识别目标、约束、证据强度、关键假设、可推翻条件和行动边界。

= 二、概念定义
<二概念定义>
== 2.1 AI 谄媚
<ai-谄媚>
本文所说的 AI 谄媚不是礼貌、个性化或表达友善。它指的是一种
truth-tracking
failure：模型为了顺应用户问题、偏好、情绪或主流叙事，降低了对事实、反证、约束和不确定性的忠诚度。
...
== 3.5 数学方法的可操作化
<数学方法的可操作化>
本文使用数学优化不是为了制造精确幻觉，而是为了给开放式调研提供一套反偏见语言。现实决策无法被一个公式完全描述，但公式能提醒
agent：目标、约束、风险、不确定性和机会成本必须同时进入判断。
...
agent
又可能沿着空头路径收敛。两种回答都可能逻辑自洽，但都只是从不同初始点得到的局部解。反谄媚调研的第一步，就是不把用户初始点误认为目标函数本身。

=== 3.5.2 多初始点搜索
<多初始点搜索>
Multi-start
的思想是从多个初始点启动搜索，比较不同路径的解。映射到调研中，就是让
agent
从多头、空头、中立、风险管理者、长期投资者、短线交易者等立场分别分析。多视角的价值不在于平均各方观点，而在于发现结论对初始立场是否敏感。
...
=== 3.5.7 正则化
<正则化>
机器学习中的正则化用于降低过拟合。开放式调研中的正则化用于防止 agent
过度拟合热门叙事、用户偏好或近期数据。
...
这些叙事可能有事实基础，但如果报告只沿着这些叙事展开，就会过拟合。反叙事正则项要求
agent 主动加入：
...
=== 3.5.9 决策树
<决策树>
决策树把不确定性转化为条件化行动。它不要求 agent 预测唯一未来，而是要求
agent 说明不同状态下该怎么做。
...
决策树能显著降低谄媚风险，因为它迫使 agent
把结论写成可检验、可更新、可执行的条件系统。
...
这个重构动作是反谄媚的第一步：agent
不再把用户原句当作目标函数，而是先检查用户真正要优化什么。
...
<正则化防止过度拟合热门叙事>
热门叙事会让 agent
输出流畅但脆弱的答案。正则化的作用是主动加入会约束叙事的变量。
...
= 五、最小评估框架
<五最小评估框架>
为了让本文不只是方法主张，需要给出可检查的评估标准。下面是一个轻量评估方案，适合用于
agent 输出质量检查。
...
第四，它不能替代专业责任。投资、医疗、法律等高风险领域仍需要专业判断和责任边界。AI
可以组织证据和推理，但不应伪装成最终责任主体。

第五，它仍可能被形式主义侵蚀。如果 agent
只是机械填表，把每个环节变成审计清单，报告仍然会失去可读性。真正的目标是服务人类决策者，而不是证明
agent 做过很多步骤。
...
十二、最终报告要求
正文应写成给人类决策者阅读的报告，先讲判断逻辑，再放证据细节。
证据登记、审计表、来源清单和过程说明可以进入附录或过程文件。
结论应服务人类决策者，而不是成为任务过程审计。

= 八、结论
<八结论>
开放式调研最危险的失败模式，不是 agent
没有找到足够多材料，而是它太快接受了用户问题里的初始方向。它看似在帮助用户，实际上可能只是把用户的初始偏见扩写成一份结构完整的报告。
