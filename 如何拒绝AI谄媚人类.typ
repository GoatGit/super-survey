...
= 五、最小评估框架
<五最小评估框架>
为了让本文不只是方法主张，需要给出可检查的评估标准。下面是一个轻量评估方案，适合用于智能体输出质量检查。
...
第五，它仍可能被形式主义侵蚀。如果智能体只是机械填表，把每个环节变成审计清单，报告仍然会失去可读性。真正的目标是服务人类决策者，而不是证明智能体做过很多步骤。
...
= 八、结论
<八结论>
开放式调研最危险的失败模式，不是智能体没有找到足够多材料，而是它太快接受了用户问题里的初始方向。它看似在帮助用户，实际上可能只是把用户的初始偏见扩写成一份结构完整的报告。
