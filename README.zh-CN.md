...
Super Survey 是一个可复用的 agent skill，用于多轮产品、市场、技术、开源和尽调类调研。它把模糊问题转化为带约束的决策优化工作流：先重建目标函数、约束和隐含假设，再通过证据、反方挑战、综合判断和下一轮更具体问题，产出可落盘、可复核、可阅读的 Markdown 报告。它面向兼容 Skills 的 agent，也可以直接通过内置 CLI 使用。
...
Super Survey 优先使用事前引导，而不是依靠事后提高门限来补救薄弱调研：在证据收集前先定义目标、约束、决策关键变量、最低直接证据、隐含预期和反叙事正则项。

## 理论基础

项目内置 [如何拒绝AI谄媚人类.md](如何拒绝AI谄媚人类.md) 作为本技能的反谄媚基础理论论文。论文用投资案例说明一个通用原则：开放式调研应被视为带约束的决策优化，而不是直接生成答案。Super Survey 将这个思想泛化到产品、市场、技术、开源和尽调场景，在收敛前先重建目标函数、约束、隐含预期、反事实、情景和更新规则。
...
CLI 可以一次创建所有轮次脚手架，但内容填写有严格依赖顺序：先写 `NN-research.md`，再写 `NN-brainstorm.md`、`NN-redteam.md`、`NN-synthesis.md`，最后写 `NN-evolver.md`。下游文件应引用已经写入的上游发现，而不是预判最终结论。
...
```mermaid
flowchart TD
    A[用户调研问题] --> B[00-brief.md<br/>决策、优化合约、研究镜头、研究框架、证据标准]
    ...
    F --> G[综合结论<br/>置信度、决策依据、敏感变量、反事实]
    G --> H[进化器<br/>保留 / 收窄 / 转向 / 放弃 / 最终成稿<br/>Kill 边界和原始问题状态]
    H --> Q{进化器决策}
