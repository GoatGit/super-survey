...
当最新决策是 `Keep`、`Narrow` 或 `Pivot` 时，`check` 可以带继续提示通过，方便进入下一轮，而不是诱导 agent 过早改成 `Kill`。轮次必须是正整数。

## 模式与证据登记

模式选择必须显式。默认使用 `standard`。只有用户明确要求“快速 / 简短 / 初筛 / 先扫一遍 / 低风险方向性扫描”时才用 `quick`；用户明确要求“深度 / 正式 / 长报告 / 大量引用 / 严格审计 / 出版级交付 / 关键决策备忘录”时用 `deep`。

| 模式 | 适用场景 | 最低登记要求 | 报告门限 |
|---|---|---:|---|
| `quick` | 用户明确要求快速、简短、初筛或低风险方向性扫描 | 1 个来源、1 个主张、1 条证据 | 分数 >=80 |
| `standard` | 默认可复用调研报告，包括普通“调研 X 是否值得做 / 买 / 采用”请求 | 3 个来源、3 个主张、3 条证据 | 分数 >=90 |
| `deep` | 用户明确要求深度、正式、长报告、大量引用、严格审计、出版级交付或关键决策备忘录 | 8 个来源、6 个主张、8 条证据 | 分数 >=95 |

在 `quick` 模式下，如果 `NN-round.md` 已包含本轮问题、证据计划、证据与来源、brainstorming 检查点、反方挑战、综合结论、原始决策和下一步，它可以替代标准/深度模式下的拆分轮次产物。

不要用 `quick` 作为高风险行动决策的最终交付模式，例如投资、法律、医疗、安全、生产事故或重大商业承诺。此类问题应使用 `standard` 或 `deep`，让阶段产物、证据覆盖和质量门与决策风险匹配。

如果高风险行动请求里同时出现“快速”之类的速度要求，模式仍使用 `standard` 或 `deep`，只把最终摘要写短一些，不把工作流降级成 `quick`。

如果请求有歧义，可先运行 `python3 scripts/survey_round.py recommend-mode --text "<request>"` 再执行 `init`。
...
| 节点 | 承担的工作 | 对应论文方法 |
|---|---|---|
| Brief / Frame Contract | 保留原始问题，重构目标函数，拆分事实、假设、推断和价值判断，定义约束、候选行动和研究框架。 | 4.1, 4.2, 4.3, 6.2 |
| Evidence Plan / Minimum Direct Evidence | 在收集来源前定义本轮要压缩的目标残差、决策关键变量、最低直接证据、优先来源类型、反证证据、缺失证据处理和框架维度的证据需求。 | 3.7.3, 3.7.4, 5.2, 5.3, 6.2 |
| Research | 按证据计划收集当前来源和一手证据，更新来源、主张和证据登记，记录矛盾、置信度、时效性和框架覆盖。 | 4.2, 4.5, 4.7, 4.9, 5.1 |
| Post-Research Brainstorming | 在证据存在后重新打开候选解释，比较多初始点视角，识别可能错误、下一步证据动作和由证据触发的框架修正。 | 4.4, 4.5, 4.6, 5.3 |
| Redteam | 攻击当前最强论证、替代方案、隐藏假设、放弃条件和反叙事正则项。 | 4.5, 4.8, 4.12, 5.2 |
| Synthesis | 把证据和反方挑战整合成敏感性分析、隐含预期反推、贝叶斯更新、情景、决策树和约束分支建议。 | 4.6, 4.7, 4.9, 4.10, 4.11 |
| Evolver | 评估残差，比较 VOI 与调研成本，决定 Keep / Narrow / Pivot / Kill / Final，区分未来事实和仍可桌面研究的缺口，生成下一轮目标或最终化理由。 | 3.7.1-3.7.4, 5.2-5.5, 6.2-6.5 |
| Final Report | 写成可独立阅读的人类决策备忘录：框架正文章节、决策逻辑、建议、变化触发器、下一步、边界和附录。 | 4.10, 4.11, 6.3-6.5 |
...
- `check` 是轮次门：检查轮次产物、证据登记链接和弱支撑、研究框架覆盖及显式修正、必要的 companion 记录和最新进化器原始决策。当决策是 `Keep`、`Narrow` 或 `Pivot` 时，它可以带继续提示通过。
- 进化器是方向门：`Keep`、`Narrow`、`Pivot` 表示创建下一轮并更新 `index.md`；`Final` 表示可以进入最终报告写作；`Kill` 表示当前论点应停止或转向非桌面研究验证。
- 残差 / VOI / 硬约束门是停止安全门：最新进化器和 `index.md` 必须记录 `r_q/r_c/r_e/r_h/r_a/r_s/r_j` 0-3 分、目标残差、预期信息价值、调研成本、VOI 是否大于成本，以及硬约束是否满足。
- `check-final` 是交付门：要求完整的正文优先 `report.md`、`index.md` 中记录的质量分通过对应模式门限，残差门通过，硬约束门通过，并且最新进化器原始决策是 `Final` 或 `Kill`。
...
模式门限是硬门：`quick >=80`、`standard >=90`、`deep >=95`。最终报告低于所选模式门限时，必须围绕最低分维度继续下一轮。helper 只用进化器原始决策、质量分门限、残差门和硬约束门判断是否停止，不把 `report.md` 里的“未来披露”或“外部验证”等解释性文字当成停止规则。

残差向量是论文“残差驱动的证据迭代”的操作化版本。`r_q` 表示问题框架，`r_c` 表示约束，`r_e` 表示证据，`r_h` 表示假设，`r_a` 表示对抗检验，`r_s` 表示敏感性，`r_j` 表示行动可执行性。任何残差为 `3` 都是决策级缺口。若存在可通过桌面研究降低该缺口的具体动作，且 VOI 大于成本，就继续；只有当没有残差为 `3`、硬约束通过、剩余桌面研究 VOI 低于成本时，才进入最终报告。

原始进化器决策是机器可读行，不是本地化散文。无论报告语言是什么，`决策` 标题下的第一条非空行都必须严格写成 `Keep`、`Narrow`、`Pivot`、`Kill` 或 `Final` 之一；中文解释写在下一行之后。
...
flowchart TD
    ...
    H --> V{残差 / VOI / 硬约束}
    V -->|残差为3 + VOI > 成本| I[index.md<br/>工作台：下一轮目标和为何未最终成稿]
    I --> P
    V -->|Final / Kill + 门通过| J[写 report.md<br/>框架维度作为正文章节]
    J --> R[check-final<br/>分数、残差、硬约束和正文优先门]
    R -->|任一门未通过| I
    R -->|全部通过| K[最终回答<br/>决策导向摘要]
```
...
| 决策 | 保留或丢弃代码改动 | `Keep` 保持、`Narrow` 收窄、`Pivot` 转向、`Kill` 放弃或 `Final` 最终成稿 |
