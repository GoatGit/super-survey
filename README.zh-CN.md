...
`研究镜头` 决定哪些证据需要被重点关注；`研究框架` 告诉读者报告按什么方法系统展开。每次调研都应写明采用的框架、框架维度，以及哪些维度薄弱或被有意排除。
...
- `check` 是轮次门：检查轮次产物、证据登记链接、研究框架覆盖、companion/wiki 记录和最新进化器原始决策。当决策是 `保留`、`收窄` 或 `转向` 时，它可以带继续提示通过。
...
最终报告应该像人类写的判断备忘录：先给答案、阅读路径、研究方法与框架、正文叙事、决策逻辑、最终建议、改变结论的触发条件、下一步行动和报告边界；证据登记表、来源质量、反方挑战、情景分析、质量分和来源清单放在附录里。
