...
进化器在报告质量评分之前运行。它是轮次级门限，负责把最新综合结论和反方挑战转成 `保留 / 收窄 / 转向 / 放弃`，并产出更锋利的下一轮焦点。质量分是报告级门限，只打在更新后的 `report.md` 上。调研只有在两个门都通过时才可以停止：报告达到所选模式门限，且进化器要么用明确 kill reason 放弃当前论点，要么已经没有可通过桌面调研继续降低的下一证据。如果进化器给出 `保留`、`收窄` 或 `转向`，并命名了可桌面调研的证据，就必须继续下一轮，除非 `report.md` 明确解释该证据只能靠外部验证获得，或已不再影响决策。
