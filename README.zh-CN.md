...
- `check-final`：在 `check` 的基础上，额外检查最终 `report.md`、正文优先规则、对应模式的质量分，以及最新进化器原始决策必须是 `放弃`。
...
- `check-final` 是交付门：要求完整的正文优先 `report.md`、通过对应模式的质量分，并且最新进化器原始决策是 `放弃`。
...
最终报告应该像人类写的判断备忘录：先给答案、阅读路径、研究方法与框架、框架维度分析、正文叙事、决策逻辑、最终建议、改变结论的触发条件、下一步行动和报告边界；证据登记表、来源质量、反方挑战、情景分析、质量分和来源清单放在附录里。只要方法部分列出了维度，这些维度就必须以 Markdown 子标题出现在正文里，不能只停留在方法说明或附录审计内容中。正文如果被项目符号或审计表格主导，不能通过最终门。
